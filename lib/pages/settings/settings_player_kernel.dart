...
class _SettingsPlayerKernelState extends State<SettingsPlayerKernel> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(AppLocalizations.of(context)!.settingsItemPlayerKernel)),
      body: ListView(
        children: [
          RadioListTile(
  ...
  Widget _buildMpvListTile(_MpvLib lib, String? mpvVersion) {
    ...
    if (lib.downloaded && lib.version != mpvVersion) {
      return Dismissible(
        key: ValueKey(lib.version),
        confirmDismiss: (_) async {
          final confirmed = await showConfirm(context, AppLocalizations.of(context)!.deleteConfirmText) ?? false;
          if (confirmed) {
            await _deleteMpvLib(lib.version);
          }
          return confirmed;
        },
        onDismissed: (_) {
          _localMpvLibs.remove(lib);
          _remoteMpvLibs?.where((it) => it == lib).forEach((it) {
            it.downloaded = false;
          });
          setState(() {});
        },
